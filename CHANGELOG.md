...
- Automatic stops (silence timeout, max duration) are fully silent, even when
  they deliver text, and their restarts play no start sound. Audible cues come
  only from a key stop or the send phrase, so an open mic in continuous mode
