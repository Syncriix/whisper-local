# terminal_title.py
# Shows app state in the terminal's tab/window title ("<prefix> Whisper Local")
# via the OSC 0 escape sequence. A second status surface for people who run the
# app in a visible terminal: the tab stays readable when the tray icon is hidden
# behind an overflow arrow and the overlay is off screen.
#
# Each state maps to either a static prefix or a list of [prefix, seconds] frames
...
_APP_TITLE = "Whisper Local"

# A static prefix still needs a wait interval for the animation loop; make it
# long enough that a non-animated state costs essentially nothing.
...
class TerminalTitle:
    def __init__(self, frames_config: dict = None):
        ...
        # isatty() can raise on a closed/replaced stream, and stdout is None
        # under pythonw — treat any of that as "no terminal to title".
        try:
        ...
        self._thread = None
        if self._enabled:
            # Emit immediately so the title is correct before the loop starts.
    ...
    # Accepts a scalar (static prefix) or a list of [prefix, seconds] frames.
    # Malformed entries are dropped rather than raised: a typo in the config
    # must not stop the app from starting, so we fall back to the default.
    ...
    def start(self):
        ...
        self._thread = threading.Thread(
            target=self._animation_loop, daemon=True, name="TerminalTitle")
        self._thread.start()

    # Called from the state machine on every transition. Setting the tick event
    # wakes the animation loop immediately so the title changes without waiting
    # out the current frame's interval.
    ...
    def stop(self):
        ...
        self._emit("")  # hand the tab title back to the shell
    ...
    def _emit(self, title: str):
        try:
            ...
            # Stream went away (console closed, pipe broken) — stop trying.
            self._stop.set()
